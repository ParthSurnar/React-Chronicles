...
const Shorten = () => {
    ...
    const generate = () => {
        // Check if the URL and short URL are empty
        if (url === "" || shorturl === "") {
            alert("Please Enter URL and Short URL");
        ...
        fetch("/api/generate", requestOptions)
            ...
            .then((result) => {
                ...
                setshorturl("");  // Clear the input fields
                if (result.success) {
                    // Show success message only when URL generation is successful
                    alert("URL Generated Successfully");

                } else if (result.error) {
                    // Show error message if the URL already exists
                    alert(result.message); // You can show the specific message from the result here
                }
            })
  ...
  placeholder-shown:opacity-50 focus:placeholder-opacity-100 focus:translate-y-[-3px]"
                    ...
                    <button onClick={generate} className="relative rounded-xl px-6 py-3 mt-2 text-lg font-semibold text-white bg-gradient-to-r from-purple-600 to-purple-500 shadow-md shadow-purple-400 transition-all duration-300
